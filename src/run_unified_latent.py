...
def run(start: int, count: int, mode: str) -> dict:
    ...
    for sample_id in range(start, start + count):
        ...
        if path.exists(): statuses.append(json.loads(path.read_text())["sanity"]); continue
    ...
    result = {"mode":mode,"start":start,"count":count,"completed":len(statuses),"all_passed":len(statuses)==count and all(s.get("passed",False) for s in statuses),"samples":statuses,"config":config.as_dict()}
